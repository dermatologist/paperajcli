...
export function processLatex(content: string): string {
  ...
  for (let line of lines) {
    ...
    // Fix for MS Word: convert '\textbackslash{}cite{...}' or '\textbackslash cite{...}' to '\cite{...}'
    if (line.includes(String.raw`\textbackslash`)) {
      // Handles both with and without braces after \textbackslash, and possible invisible Unicode chars (e.g. LRM, RLM, ZWSP)
      // MS Word may insert invisible chars between tokens, so match them as well
      // Unicode: \u200E (LRM), \u200F (RLM), \u200B (ZWSP), \u202A-\u202E (directional marks)
